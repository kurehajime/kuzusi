out:
	...
	rm -rf ./_obj
	PATH="$(shell go env GOPATH)/bin:$$PATH" gox -osarch="linux/386 linux/amd64 linux/arm windows/386 windows/amd64 darwin/amd64 darwin/arm64" -output "_obj/{{.OS}}_{{.Arch}}/{{.Dir}}" .
